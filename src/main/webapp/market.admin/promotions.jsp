    ...
    <c:forEach var="promotion" items="${promotions}">
      <tr class="h-10 even:bg-gray-50">
        ...
        <td class="p-3" x-data="{status: '${promotion.getStatus()}'}">
          <div x-bind:class="status == 'refused' ? 'bg-red-100 text-red-600' : status == 'accepted' ? 'bg-green-100 text-green-600 animate-pulse' : 'bg-[#d8b4fe] text-[#6b21a8]'" class="rounded-full px-4 py-1 text-center w-fit font-normal">
              ${promotion.getStatus()}
          </div>
        </td>
