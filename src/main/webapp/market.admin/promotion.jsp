...
<div>
  <div class="flex justify-center items-center relative" x-data="{status: '${promotion.getStatus()}'}">
    ...
    <span class="p-1 absolute w-full top-1/2 bg-gray-50 rounded-full" x-bind:class="status == 'refused' ? 'bg-red-100 text-red-600' : status == 'accepted' ? 'bg-green-100 text-green-600' : status == 'pending' ? 'bg-blue-100 text-blue-600 animate-pulse' : 'bg-[#d8b4fe] text-[#6b21a8]'"></span>
  </div>
